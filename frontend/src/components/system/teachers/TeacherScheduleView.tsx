...
import { ChevronLeft, ChevronRight, Clock, LayoutGrid, Filter } from 'lucide-react'
...
type ViewFilter = 'all' | 'available' | SessionStatus
...
const STATUS_CFG: Record<string, { bg: string; border: string; text: string; accent: string; label: string }> = {
  scheduled: {
    bg: 'rgb(14 124 90 / 0.09)', border: 'rgb(14 124 90 / 0.55)',
    text: 'rgb(6 54 39)', accent: 'rgb(14 124 90)', label: 'Confirmed',
  },
  attended: {
    bg: 'rgb(34 197 94 / 0.10)', border: 'rgb(22 163 74 / 0.50)',
    text: 'rgb(15 118 54)', accent: 'rgb(22 163 74)', label: 'Attended',
  },
  absent: {
    bg: 'rgb(245 158 11 / 0.10)', border: 'rgb(245 158 11 / 0.55)',
    text: 'rgb(120 53 15)', accent: 'rgb(245 158 11)', label: 'Absent',
  },
  cancelled: {
    bg: 'rgb(148 163 184 / 0.10)', border: 'rgb(148 163 184 / 0.45)',
    text: 'rgb(71 85 105)', accent: 'rgb(148 163 184)', label: 'Cancelled',
  },
  rescheduled: {
    bg: 'rgb(148 163 184 / 0.10)', border: 'rgb(148 163 184 / 0.45)',
    text: 'rgb(71 85 105)', accent: 'rgb(148 163 184)', label: 'Rescheduled',
  },
  pending_substitute: {
    bg: 'rgb(239 68 68 / 0.09)', border: 'rgb(239 68 68 / 0.55)',
    text: 'rgb(153 27 27)', accent: 'rgb(239 68 68)', label: 'Needs Sub',
  },
}
...
const FILTER_OPTIONS: { value: ViewFilter; label: string; color: string }[] = [
  { value: 'all',                label: 'All',        color: 'rgb(100 116 139)' },
  { value: 'available',          label: 'Available',  color: 'rgb(14 124 90)'   },
  { value: 'scheduled',          label: 'Confirmed',  color: 'rgb(14 124 90)'   },
  { value: 'attended',           label: 'Attended',   color: 'rgb(22 163 74)'   },
  { value: 'absent',             label: 'Absent',     color: 'rgb(245 158 11)'  },
  { value: 'pending_substitute', label: 'Needs Sub',  color: 'rgb(239 68 68)'   },
  { value: 'cancelled',          label: 'Cancelled',  color: 'rgb(148 163 184)' },
]
...
export function TeacherScheduleView({ teacherId, availability }: Props) {
  ...
  return (
    <div className="rounded-2xl overflow-hidden border" style={{ background: surface, borderColor: border }}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 px-5 py-4 border-b" style={{ borderColor: border }}>
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-0.5">
            <button onClick={() => setWeekStart(w => addDays(w, -7))} className="p-1.5 rounded-lg hover:bg-black/5 transition-colors">
              <ChevronLeft size={16} />
            </button>
            <button
        ...
        <div className="flex flex-wrap items-center gap-4 text-[11px] font-medium" style={{ opacity: 0.6 }}>
          <LegendChip shape="rect" color="rgb(14 124 90 / 0.35)" border="rgb(14 124 90 / 0.6)" label="Available" />
      ...
      {/* ── Filter bar ── */}
      <div className="flex flex-wrap items-center gap-1.5 px-5 py-3 border-b" style={{ borderColor: border, background: 'rgb(var(--surface-card-2, 248 250 252))' }}>
        <span className="flex items-center gap-1.5 text-[11px] font-semibold mr-1" style={{ opacity: 0.5 }}>
          <Filter size={11} /> Filter
        </span>
        {FILTER_OPTIONS.map(opt => {
          const active = viewFilter === opt.value
          return (
            <button
              key={opt.value}
              ...
              style={active
                ? { background: opt.color, color: '#fff', boxShadow: `0 0 0 2px ${opt.color}` }
                : { background: 'transparent', color: opt.color, border: `1.5px solid ${opt.color}40` }
              }
            >
              {opt.value !== 'all' && opt.value !== 'available' && (
                <span className="w-2 h-2 rounded-full shrink-0" style={{ background: active ? '#fff' : opt.color }} />
              )}
              {opt.label}
            </button>
          )
      ...
      <div className="grid grid-cols-3 border-b" style={{ borderColor: border }}>
        <StatCell icon={<LayoutGrid size={13} />} label="Weekly available" value={fmtDuration(totalAvailMin)} color="rgb(14 124 90)"  border={border} rightBorder />
        <StatCell icon={<Clock size={13} />}       label="Booked this week" value={fmtDuration(bookedMin)}    color="rgb(99 102 241)" border={border} rightBorder />
        <StatCell icon={<Clock size={13} />}       label="Free slots"       value={fmtDuration(freeMin)}      color={freeMin === 0 ? 'rgb(239 68 68)' : 'rgb(14 124 90)'} border={border} />
      </div>
        ...
        <div style={{ minWidth: 700 }}>
          ...
          <div className="flex sticky top-0 z-20 border-b" style={{ background: surface, borderColor: border }}>
            ...
            {DAYS.map((label, dow) => {
              ...
              return (
                <div key={dow} className="flex-1 flex flex-col items-center py-3 select-none" style={{ borderLeft: `1px solid ${border}` }}>
                  <span className="text-[10px] uppercase tracking-widest font-semibold" style={{ color: isToday ? 'rgb(14 124 90)' : undefined, opacity: isToday ? 1 : 0.4 }}>
                    {label}
                  </span>
                  <span
          ...
          <div ref={scrollRef} className="overflow-y-auto relative" style={{ maxHeight: 560, paddingTop: 14 }}>
            {isLoading && (
              <div className="absolute inset-0 z-30 flex items-center justify-center" style={{ background: 'rgb(255 255 255 / 0.8)' }}>
            ...
            <div className="flex relative" style={{ height: (GRID_END - GRID_START) * HOUR_H }}>
              ...
              {DAYS.map((_, dow) => {
                ...
                return (
                  <div key={dow} className="flex-1 relative" style={{ borderLeft: `1px solid ${border}` }}>
                    ...
                    {daySlots.map((slot, i) => {
                      ...
                      const emphasised = viewFilter === 'available'
                      ...
                      return (
                        <div
                          ...
                          style={{
                            top,
                            height,
                            background:  emphasised ? 'rgb(14 124 90 / 0.18)' : 'rgb(14 124 90 / 0.10)',
                            borderLeft:  emphasised ? '4px solid rgb(14 124 90 / 0.80)' : '3px solid rgb(14 124 90 / 0.45)',
                            zIndex:      emphasised ? 5 : 1,
                            pointerEvents: 'none',
                          }}
                        >
                          {height >= 28 && (
                            <span
                              className="absolute top-1 left-1.5 text-[9px] font-bold uppercase tracking-widest select-none"
                              style={{ color: 'rgb(14 124 90)', opacity: emphasised ? 0.85 : 0.55 }}
                            >
                              Available
                            </span>
                          )}
                    ...
                    {isToday && nowOk && (
                      <div className="absolute left-0 right-0 z-20 pointer-events-none" style={{ top: nowY }}>
                        <div className="relative flex items-center">
                          <div className="absolute rounded-full" style={{ width: 9, height: 9, background: 'rgb(14 124 90)', left: -4, top: -4 }} />
                    ...
                    {daySessions.filter(s => {
                      if (viewFilter === 'available') return false
                      ...
                      return (
                        <div
                          ...
                          style={{ top: top + 1, height: height - 2, background: cfg.bg, border: `1.5px solid ${cfg.border}` }}
                        >
                          {/* left accent bar */}
                          <div className="absolute left-0 top-0 bottom-0" style={{ width: 3, background: cfg.accent, borderRadius: '6px 0 0 6px' }} />

                          <div className="pl-3 pr-2 py-1 h-full flex flex-col justify-center gap-0.5">
                            {compact ? (
                              <div className="flex items-center gap-1.5 min-w-0">
                                <span className="shrink-0 px-1 py-px rounded text-[8px] font-bold uppercase tracking-wide leading-none"
                              ...
                              <>
                                <div className="flex items-center gap-1.5 min-w-0">
                                  <span className="shrink-0 px-1.5 py-px rounded text-[8px] font-bold uppercase tracking-wide leading-none"
                                    style={{ background: kindCfg.bg, color: kindCfg.color }}>
                                    {kindCfg.label}
                                  </span>
...
function LegendChip({ color, label, shape = 'circle', border }: { color: string; label: string; shape?: 'circle' | 'rect'; border?: string }) {
  return (
...
function StatCell({ icon, label, value, color, border, rightBorder }: {
  icon: React.ReactNode; label: string; value: string; color: string; border: string; rightBorder?: boolean
}) {
  return (
    <div className="py-3.5 px-4 flex flex-col items-center gap-0.5" style={rightBorder ? { borderRight: `1px solid ${border}` } : {}}>
