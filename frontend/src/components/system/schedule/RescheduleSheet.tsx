...
const CONFLICT_LABEL_KEYS: Record<string, string> = {
  teacher_double_booking: 'schedule.conflict.teacherDoubleBooking',
  teacher_on_leave:       'schedule.conflict.teacherOnLeave',
  teacher_unavailable:    'schedule.conflict.teacherUnavailable',
}
    ...
    <>
      ...
      <div className="fixed right-0 top-0 bottom-0 z-70 w-full max-w-sm bg-background shadow-2xl border-l flex flex-col p-4">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold">{t('schedule.reschedule.title')}</h3>
          <button onClick={onClose} className="p-1 rounded hover:bg-muted">×</button>
        </div>

        <div className="space-y-4 flex-1">
          <div>
            <label className="text-sm font-medium">{t('schedule.reschedule.newDateTime')}</label>
            <input
              type="datetime-local"
          ...
          <div className="text-xs text-muted-foreground">
            {t('schedule.reschedule.durationLocked', { minutes: String(session.duration_min) })}
          </div>
          ...
          >
            {preview.isPending ? t('schedule.reschedule.checking') : t('schedule.reschedule.checkConflicts')}
          </button>

          {conflicts.length > 0 && (
            <div className="rounded-md bg-orange-50 border border-orange-200 p-3 space-y-2">
              <div className="text-xs font-medium text-orange-800">⚠ {t('schedule.conflict.detected')}</div>
              {conflicts.map((c, i) => (
                <div key={i} className="text-xs text-orange-700">{CONFLICT_LABEL_KEYS[c.type] ? t(CONFLICT_LABEL_KEYS[c.type]) : c.type}</div>
              ))}
              {!hasHardBlock && (
                <label className="flex items-center gap-2 mt-2 cursor-pointer">
                  ...
                  />
                  <span className="text-xs font-medium text-orange-800">{t('schedule.conflict.saveAnyway')}</span>
                </label>
              )}
            </div>
          )}
          ...
          {previewData && conflicts.length === 0 && (
            <div className="rounded-md bg-green-50 border border-green-200 p-3 text-xs text-green-800">
              ✅ {t('schedule.reschedule.noConflicts')}
            </div>
          )}
        </div>
        ...
        <div className="flex gap-2 pt-4 border-t mt-4">
          ...
          >
            {reschedule.isPending ? t('common.saving') : t('schedule.reschedule.save')}
          </button>
        </div>
      </div>
    </>
