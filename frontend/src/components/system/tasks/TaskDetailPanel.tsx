...
export function TaskDetailPanel({ taskId, canDecide, onClose }: Props) {
  ...
  return (
    <>
      <div className="fixed inset-0 z-40 bg-[#18483C]/40 backdrop-blur-sm" onClick={onClose} />
        ...
        {task && !isLoading && (
          <div className="shrink-0 border-t bg-white px-4 py-3 flex items-center gap-2" style={{ borderColor: 'rgb(229 233 240)' }}>
            ...
            {canAct ? (
              ...
              <button onClick={onClose} className="px-4 py-2 rounded-lg text-xs font-semibold text-white" style={{ background: '#18483C' }}>
                {t('tasks.close')}
              </button>
            )}
          </div>
